...
import { 
  ...
  Settings, Save, Phone, MapPin, Info, CheckCircle2, 
  Edit3, Check, X 
} from 'lucide-react';
...
const FarmerProfile = ({ farmer, onBack }) => {
  ...
  // State for Editing Farmer Rates
  ...
  const saveMilkEdit = async (id) => {
    try {
      ...
      fetchFarmerData(); // Refresh totals and table
    ...
    <div className="w-full animate-in slide-in-from-right-10 duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-3 space-y-6">
            ...
            <QuickStat label="Gross Earnings" value={`₹${milkData.reduce((a,c) => a+c.totalAmount, 0).toLocaleString()}`} icon={<Landmark className="text-emerald-500"/>} />
        ...
        <div className="lg:col-span-9 bg-white rounded-[3rem] shadow-xl border border-slate-100 overflow-hidden flex flex-col">
            <div className="flex bg-slate-50/50 border-b">
                ...
                <TabBtn active={activeTab === 'settings'} onClick={() => setActiveTab('settings')} label="Rates & Logic" icon={<Settings size={16}/>} />
            </div>
                                ...
                                {milkData.map(m => (
                                    <tr key={m.id} className="text-sm font-bold text-slate-700 hover:bg-slate-50/50 transition-colors group">
                                        <td className="py-5">
                                            ...
                                            <span className="text-[10px] uppercase opacity-30 ml-2">{m.shift}</span>
                                        </td>
                                        ...
                                        <td className="py-5 text-center">
                                            {editingMilkId === m.id ? (
                                                ...
                                                <span className={`text-xs font-black ${m.fat === 0 ? 'text-red-400 italic' : 'text-slate-400'}`}>
                                                    {m.fat > 0 ? `${m.fat} / ${m.snf}` : 'Missing Quality'}
                                                </span>
                                            )}
                                        </td>
                                        ...
                                        <td className="py-5 text-right">
                                            {editingMilkId === m.id ? (
                                                <div className="flex justify-end gap-2">
                                                    <button onClick={() => saveMilkEdit(m.id)} className="p-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 shadow-md transition-all"><Check size={16}/></button>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {advanceData.map(a => (
                                    <tr key={a.id} className="text-sm font-bold text-slate-700">
                                        <td className="py-5">{new Date(a.date).toLocaleDateString()}</td>
                                        <td className="py-5 text-slate-500 font-medium">{a.description || 'General Advance'}</td>
                                        <td className="py-5 text-right text-red-600 font-black">₹{a.amount.toLocaleString()}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                {activeTab === 'settings' && (
                    <div className="animate-in fade-in zoom-in-95 duration-300 max-w-2xl">
                        <form onSubmit={handleUpdateProfile} className="space-y-8">
                            <div className="space-y-4">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Rate Calculation Logic</label>
                                <select 
                                    value={editForm.rateType} 
                                    onChange={e => setEditForm({...editForm, rateType: e.target.value})}
                                    className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold text-slate-900 outline-none"
                                >
                                    <option value="FAT_SNF">Fat + SNF Combined</option>
                                    <option value="FAT_ONLY">Fat Based Only</option>
                                    <option value="FIXED">Flat Fixed Rate / Liter</option>
                                </select>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-500 uppercase">Fat Price (₹)</label>
                                    <input type="number" step="0.01" value={editForm.fatRate} onChange={e => setEditForm({...editForm, fatRate: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-black" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-500 uppercase">SNF Price (₹)</label>
                                    <input type="number" step="0.01" value={editForm.snfRate} onChange={e => setEditForm({...editForm, snfRate: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-black" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-500 uppercase">Fixed Price (₹)</label>
                                    <input type="number" step="0.01" value={editForm.fixedRate} onChange={e => setEditForm({...editForm, fixedRate: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl font-black text-blue-600" />
                                </div>
                            </div>
                            <button type="submit" className="w-full bg-slate-900 hover:bg-black text-white font-black py-5 rounded-3xl shadow-xl flex items-center justify-center gap-3 transition-all active:scale-95 group">
                                <Save size={22} className="group-hover:scale-110 transition-transform" /> SAVE NEW SETTINGS
                            </button>
                        </form>
                    </div>
                )}
...
const TabBtn = ({ active, onClick, label, icon }) => (
    <button onClick={onClick} className={`flex-1 flex items-center justify-center gap-3 py-6 font-black uppercase tracking-widest text-[10px] transition-all ${active ? 'bg-white text-blue-600 border-b-2 border-blue-600' : 'bg-slate-50/50 text-slate-400 hover:bg-slate-100'}`}>
        {icon} {label}
    </button>
);
