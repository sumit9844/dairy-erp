...
const Sales = () => {
  ...
  const finalizeWholesaleBill = async () => {
    ...
    try {
        for (const item of billItems) {
            await axios.post('https://dairy-erp-backend.onrender.com/api/sales', {
                customerName: customerName, productName: item.productName, quantity: item.quantity, rate: item.rate, date: new Date().toISOString().split('T')[0]
            });
        }
        setCurrentInvoice({ customer: customerName, items: billItems, total: billItems.reduce((acc, item) => acc + item.total, 0), date: new Date().toLocaleDateString() });
  ...
  return (
    <div className="w-full animate-in fade-in duration-500 pb-20">
      ...
      {/* --- HISTORY MODE (With Checkboxes & Bulk Delete) --- */}
      {mode === 'HISTORY' && (
                ...
                <div className="flex items-center gap-4">
                    ...
                    {selectedIds.length > 0 && (
                        <button 
                            ...
                            className="bg-red-500 text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-red-600 animate-in fade-in zoom-in"
                        >
                            <Trash2 size={14}/> Delete {selectedIds.length} Items
                        </button>
                    )}
                </div>
                        ...
                        <tr className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b">
                            ...
                            <th className="px-6 py-5 w-10">
                                <button onClick={handleSelectAll} className="text-slate-400 hover:text-indigo-600">
                                    {currentHistory.length > 0 && currentHistory.every(i => selectedIds.includes(i.id)) 
                                        ? <CheckSquare size={18}/> 
                                        : <Square size={18}/>}
                                </button>
                            </th>
